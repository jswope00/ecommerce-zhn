...
class Wxpay_client_pub(Common_util_pub):
    ...
    def postXml(self, parameters=None):
        """post请求xml"""
        xml = self.createXml(parameters)
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def createXml(self, parameters):
        """生成接口参数xml"""
        # 检测必填参数
        if any(parameters[key] is None for key in ("out_trade_no", "body", "total_fee", "notify_url", "trade_type")):
            raise ValueError("missing parameter")
        if parameters["trade_type"] == "JSAPI" and parameters["openid"] is None:
            raise ValueError("JSAPI need openid parameters")

        parameters["appid"] = WxPayConf_pub.APPID   # 公众账号ID
